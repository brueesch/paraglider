...
var tester = {
  ...
  updateGamepads: function(gamepads) {
    ...
    if (padsConnected) {
      ...
      document.querySelector('#start-to-continue').classList.add('visible');
    } else {
      document.querySelector('#no-gamepads-connected').classList.add('visible');
      document.querySelector('#start-to-continue').classList.remove('visible');
    }
  },
  ...
  updateButton: function(button, gamepadId, id) {
    ...
    if (buttonEl) { // Extraneous buttons have just a label.
      if (pressed) {
        ...
        if(id === "button-select")
        {
        	window.location = "../Splashscreen/Splashscreen.html";
        }
      } else {
